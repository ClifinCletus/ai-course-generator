...
const CreateCourse = () => {
  ...
  const { userCourseInput, setUserCourseInput } = useContext(UserInputContext); // VVV IMP: we would set the content (userCourseInput) from each of the component
  // (ie, we would add category from the selectCategory compoent , title and desc from TopicDescription.jsx and options from the SelectOption,jsx)
  ...
  /**
   * To check "Next" button (the button in the bottom along with prev (to move through the input steps as per index)) inabled or disabled based on if the values in the page are filled or selected,
   * then only can move to next page.
   * here return true means: it not have the values to be filled, so return true, its to disable "Next" button (if true, disable the next button)
   ** */
  const checkStatus = () => {
  ...
  const GenerateCourseLayout = async () => {
    setLoading(true);
    const BASIC_PROMPT =
      "Generate a Course Tutorial on The Following Detail with field as Course Name, Description, Along with Chapter Name, about, Duration: ";
