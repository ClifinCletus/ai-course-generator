            ...
            <div key={index} className='border p-5 rounded-lg mb-2 flex items-center justify-between'>
                <div className='flex gap-5 items-center'>
                    ...
                    <div>
                        ...
                        <h2 className='font-medium text-lg'>{chapter?.chapterName} <EditChapters index={index} course={course} refreshData={() => refreshData(true)}/></h2>
                        <p className='text-sm tex-gray-500'>{chapter?.about}</p>
                        <p className='flex gap-2 mt-3 text-violet-400 items-center text-sm'> <HiOutlineClock/> {chapter?.duration}</p>
                    </div>
                </div>
